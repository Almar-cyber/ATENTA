...
interface CreatePostBody {
  ...
  // Conta ausente do mapa = usa media_asset_ids, como sempre. Lista vazia NÃO é o mesmo que ausente:
  // é como se diz "este destino sai sem arquivo" sem tirar a mídia dos outros.
...
/**
 * Algum dos destinos SELECIONADOS trouxe mídia própria?
 *
 * Duas coisas não contam como conteúdo. Uma conta com lista vazia, porque `[]` é justamente como se
 * diz "este destino sai sem arquivo" quando o post tem lista compartilhada. E uma chave que não está
 * em `target_account_ids`: sem esse filtro, um mapa apontando pra uma conta que nem foi selecionada
 * fazia a guarda de "post vazio" passar, e como rascunho pula o `validate()` dos adapters, nascia um
 * post sem legenda e sem arquivo nenhum.
 */
function hasPerTargetMedia(map: Record<string, string[]> | undefined, accountIds: string[] | undefined): boolean {
  if (!map || !Array.isArray(accountIds)) return false;
  return accountIds.some((id) => Array.isArray(map[id]) && map[id].length > 0);
}
...
async function validateAccountsAndMedia(env: Env, owner: string, params: ValidateAccountsAndMediaParams): Promise<ValidateAccountsAndMediaResult> {
  ...
  // Forma do mapa conferida antes de qualquer consulta: um valor que não é lista de texto estouraria
  // lá dentro, no new Set(), e viraria 500. Um corpo malformado tem que sair como 400 legível.
  for (const [accountId, ids] of Object.entries(params.targetMediaAssetIds ?? {})) {
    if (!Array.isArray(ids) || ids.some((id) => typeof id !== 'string')) {
      return {
        ok: false,
        response: jsonResponse({ error: `target_media_asset_ids[${accountId}] precisa ser uma lista de ids` }, 400),
      };
    }
  }
  ...
  async function resolveMedia(ids: string[]): Promise<MediaAsset[] | Response> {
    ...
    // can't legally appear twice in one target, so reject it here with a readable message instead
    // of letting the insert fail mid-loop.
    ...
    for (const mediaId of ids) {
      ...
      // social de quem pediu. media_assets tem owner_id desde a migração 0007. O filtro fica AQUI,
      // dentro do resolveMedia, e não no chamador, porque agora existem dois caminhos de entrada:
      // a lista compartilhada do post e o recorte por destino (target_media_asset_ids). Os dois
      // estão cobertos por test/isolation.test.ts.
...
async function createPost(request: Request, owner: string, env: Env): Promise<Response> {
  ...
  const hasMediaOnCreate =
    (payload.media_asset_ids?.length ?? 0) > 0 ||
    !!payload.media_asset_id ||
    hasPerTargetMedia(payload.target_media_asset_ids, payload.target_account_ids);
