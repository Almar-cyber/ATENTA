...
export const youtubeAdapter: PlatformAdapter = {
  ...
  async ensureFreshToken(account, env) {
    ...
    if (!res.ok) throw await apiError('youtube: token refresh failed', res);
    ...
    if (!json.access_token) throw new Error('youtube: refresh respondeu sem access_token — resposta inesperada');
