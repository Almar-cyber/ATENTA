...
export const pinterestAdapter: PlatformAdapter = {
  ...
  async ensureFreshToken(account, env) {
    ...
    if (!res.ok) throw await apiError('pinterest: token refresh failed', res);
    ...
    if (!json.access_token) throw new Error('pinterest: refresh respondeu sem access_token — resposta inesperada');
