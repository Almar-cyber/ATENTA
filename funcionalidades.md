...
| **Reel de teste** (só pra quem não segue, gradua depois) | `src/adapters/instagram.ts` (`trial_params`), `PostComposer.tsx`, `lib/pendencias.tsx` | ⚠️ coberto por teste de ponta a ponta do nosso lado (escolha → `options` → corpo do container; o lembrete de decidir, no `/api/summary`; e a classificação de uma recusa da Meta — ver abaixo), mas **nunca publicado de verdade**. A exigência de conta profissional já é garantida pela conexão (`instagram_business_account` só existe pra Business/Criador ligada a uma Página); o piso de seguidores que circula não é publicado pela Meta e as fontes divergem entre si, de 200 a 1.000. **Se a conta não for elegível, a recusa só se sabe na publicação** — `validate()` não tem como checar isso de antemão — e `classifyError()` trata a recusa genérica (`OAuthException` sem código conhecido) como falha do POST, não da conta: falha o destino sem desconectar nem tocar na conexão (design.md, "Ciclo de vida"). Ver §6.5 |
...
| Grade 3 colunas arrastável, com permutação de horários (tela **Planejar**) | `GridPlanner.tsx`, `web/src/lib/gridOrder.ts`, `gridTiles.ts` | ✅ com teste. Destino próprio desde 08/09/2026 — era a 4ª aba da Agenda, e de lá herdava os filtros dela (ver §6.5) |
...
| ~~Arrastar não funcionava no celular~~ | `web/src/lib/usePressDrag.ts` | ✅ corrigido em 08/09/2026 — o arraste era HTML5 drag-and-drop, que **não dispara a partir do dedo** em navegador nenhum de celular. A grade do Planejar dizia "arraste para reordenar" e no telefone o dedo só rolava a página; a fila de mídia do compositor tinha o mesmo defeito. Agora o toque tem caminho próprio: pressionar e segurar 320ms pega a peça, arrastar move, soltar aplica — mesma matemática de reordenação dos dois casos. Verificado com eventos de toque reais (CDP `Input.dispatchTouchEvent`) na grade e na fila de mídia: pega, marca o destino, chama o `reschedule`, e não abre o detalhe ao soltar |
| ~~Filtro da Agenda apagava as âncoras da grade~~ | `App.tsx`, `GridPlanner.tsx` | ✅ corrigido em 08/09/2026 — enquanto o Grid IG era a 4ª aba da Agenda, ele recebia a mesma lista de posts das outras três, e `filters.status`/`filters.platform` vão pra QUERY do servidor. Filtrar por "rascunho" apagava os PUBLICADOS, que são justamente as âncoras contra as quais se planeja; filtrar por outra rede esvaziava a grade. O botão de Filtros ficava na mesma fileira das abas, apagado, sem dizer que estava ligado. Agora a tela é destino próprio (**Planejar**) e busca os próprios posts: Instagram, todos os status |
| ~~Capa em branco em tudo que já foi publicado~~ | `src/lib/csp.ts` | ✅ corrigido em 08/09/2026 — o Grid IG virava um tabuleiro de quadrados cinzas e os Insights perdiam as miniaturas. A capa dessas peças vem do FEED AO VIVO da rede (única que existe depois do purge de 30 dias, e a única que existe SEMPRE pro post importado do histórico, que nunca teve arquivo nosso), e o `img-src` da CSP que entrou em 26/08 só listava a nossa origem e o R2. Sintoma exato de bloqueio de CSP: nada quebra, nada reclama na tela, a imagem só não vem. Regressão coberta em `test/csp.test.ts`, que checa a diretiva E que ela chega na resposta do SPA |
