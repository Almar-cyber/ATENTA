...
1. **Exige conta profissional (Criador ou Empresa), e isso já está garantido antes do compositor.**
   A conta do Instagram é descoberta em `page?fields=instagram_business_account`
   (`handleMetaCallback`), campo que só existe pra conta Business ou Criador ligada a uma Página —
   conta pessoal não conecta. Toda conta no seletor já cumpre a exigência, então o compositor não a
   repete.
   Um **piso de seguidores** é outra história: a Meta não publica nenhum (o anúncio dela fala só em
   "eligible creators") e as fontes de terceiros divergem entre si, de 200 a 1.000. Citar um dos
   números dava a ele uma precisão que ele não tem e mandava desistir quem talvez pudesse — por isso
   o compositor não cita. Quem recusa, se for o caso, é a Meta na publicação; a contagem existe do
   nosso lado (`account_metrics.followers`, coletada em `src/metrics/instagram.ts`), mas sem um
   critério confiável pra comparar ela não decide nada.
