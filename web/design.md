...
| `FormatPicker` | Escolha do formato dentro da rede (Post/Reel/Story no Instagram, Vídeo/Short no YouTube). Vem **antes da mídia** no composer: é o formato que define o que a rede aceita e, no Instagram, o `media_type` do container. Os formatos ficam em `PLATFORM_FORMATS` (`lib/platforms.ts`) — não hard-code plataforma no componente. **A `hint` não descreve o formato.** Descrever ("um vídeo só", "carrossel até 10", "até 60s") era repetir o que a validação já cobra na hora certa e com o texto certo — "Anexe um arquivo", "Deixe um arquivo só — Reel não aceita carrossel", "Corte o vídeo para no máximo 1min" —, e aviso que aparece antes de haver problema é texto que se aprende a pular. Sobra só o que a validação NUNCA vai dizer, porque não é erro: uma **possibilidade** que a pessoa não descobriria (o Reel de teste: "dá pra testar o engajamento com quem não segue") ou uma **ressalva** sobre o que a escolha não faz (o Short, que o YouTube classifica sozinho). Post, Story e Vídeo ficam **sem dica nenhuma** — e sem a linha, não só sem o texto. **O Reel de teste NÃO entra aqui**: ele não muda o `media_type` (é o mesmo container de Reel com um `trial_params` a mais), e o critério pra ser formato é justamente esse. Ele mora em "Ajustes por rede", como um `Select` ("Quem vê primeiro") ao lado dos da mesma natureza — a privacidade do TikTok e o "Quem pode ver" do YouTube. |
